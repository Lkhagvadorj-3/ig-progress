...
export default function Page() {
  ...
  const viewpost = async () => {
    const response = await fetch("http://localhost:5555/posts/profile", {
      method: "GET",
      headers: {
  ...
  const viewdata = async () => {
    const response = await fetch(`http://localhost:5555/users/${user?._id}`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
    ...
    <div>
      ...
      <div className="flex flex-col gap-10">
        <div className="flex flex-col items-center">
          {" "}
          <h1 className="font-semibold text-2xl pt-10">{user?.username}</h1>
          <img
            src="/IMG_0059.jpg"
